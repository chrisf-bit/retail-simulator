...
export const ISSUE_BANK: Issue[] = [
  ...
  },
  {
    ...
    urgency: 3,
    // Tweaked per validation: the store view goes to the SM/RSD, not the buyer.
    type: "A",
    scored: true,
    title: "Markdowns concentrated on one aisle",
    description: "Produce hitting 60% markdown rate this week. The RSD has asked for a store view by end of day.",
    severity: "medium",
    tags: ["commercial"],
    icon: "trending_down",
  },
...
// Disruption bank, validated against the MVP content validation pack (DIS-*).
// `weight` (1-5) scales KPI impact; `hardness` scales the hidden-driver hit;
// `type` carries the A / A+B tag. A+B policy sign-off is not happening for the
// MVP, so everything is scored (`scored: true`); the A vs A+B distinction now
// only shapes which response is correct in scoring (see disruptionEffects), not
// whether it counts. DIS-02 (competitor flash promotion) was dropped in
// validation and is intentionally absent.
