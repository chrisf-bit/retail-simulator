...
- Per-team session tokens: `session:join` mints a random token, stores only its SHA-256 hash on the team, returns the raw token once; `session:rejoin` requires a matching token (legacy pre-token sessions allowed through). Client keeps it in **localStorage** (survives a refresh / tab close). Break-glass: `facilitator:reissue_team_token` (facilitator-gated) mints a fresh token and returns it once to the facilitator, who shares it as a recovery link (`/team/<id>?recover=<teamId>.<token>`); the team page seeds localStorage from that param then rejoins. Facilitator UI is the key icon on each coaching card.
- `facilitator:join` has a brute-force lockout (8 attempts per socket+session, then 5-min), on top of the strong random token + rate limit.
- CORS is fail-closed (allowlist of `CLIENT_ORIGIN` + localhost, never `*`) - keep `CLIENT_ORIGIN` set in prod or only localhost works.
